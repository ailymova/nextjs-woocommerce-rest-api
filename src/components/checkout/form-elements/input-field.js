...
function InputField({ handleOnChange, inputValue, name, type, label, errors, placeholder, required, containerClassNames, isShipping }) {
  const inputId = `S{name}-${isShipping ? 'shipping' : ''}`;
  return (
    <div className={containerClassNames}>
      <label htmlFor={inputId} className="leading-7 text-sm text-gray-700">
        {label || ''}
